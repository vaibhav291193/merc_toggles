...
/* For accessibility */
function initKB() {
    document.querySelectorAll('button, [type="button"]').forEach(function (el) {
...
function ButtonEventHandler(event) {
    var type = event.type;
    if (type === 'keyup' && (event.keyCode === 13 || event.keyCode === 32)) {
        event.target.classList.remove('by-keyboard');
    }
    else if (type === 'keyup' && (event.keyCode === 9)) {
        event.target.classList.remove('by-keyboard');
    }
    else if (type === 'click') {
